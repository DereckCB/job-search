...
browser under `localStorage['jobs_state']`. That is all most people need.
...
create policy "own rows" on public.job_state
...
create policy "own snapshots" on public.job_snapshots
  for all using (auth.uid() = user_id) with check (auth.uid() = user_id);
create index if not exists job_snapshots_user_ts on public.job_snapshots (user_id, created_at desc);
```
